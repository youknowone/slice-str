//! The string-like slice Pattern API.
//!
...
pub trait Pattern<'a, T: Eq>: Sized {
    /// Associated searcher for this pattern
    type Searcher: Searcher<'a, T>;

    /// Constructs the associated searcher from
    /// `self` and the `haystack` to search in.
    fn into_searcher(self, haystack: &'a [T]) -> Self::Searcher;

    /// Checks whether the pattern matches anywhere in the haystack
    #[inline]
    fn is_contained_in(self, haystack: &'a [T]) -> bool {
        self.into_searcher(haystack).next_match().is_some()
    }

    /// Checks whether the pattern matches at the front of the haystack
    #[inline]
    fn is_prefix_of(self, haystack: &'a [T]) -> bool {
    ...
    where
        Self::Searcher: ReverseSearcher<'a, T>,
    {
...
pub trait Searcher<'a, T: Eq> {
    /// Getter for the underlying string to be searched in
    ///
    /// Will always return the same `&str`
    fn haystack(&self) -> &'a [T];
...
pub trait ReverseSearcher<'a, T: Eq>: Searcher<'a, T> {
...
pub trait DoubleEndedSearcher<'a, T: Eq>: ReverseSearcher<'a, T> {}
...
#[doc(hidden)]
trait MultiCharEq<T: Eq> {
    fn matches(&mut self, c: &T) -> bool;
}

impl<T: Eq, F> MultiCharEq<T> for F
where
    T: Eq,
    F: FnMut(&T) -> bool,
{
...
struct MultiCharEqPattern<T: Eq, C: MultiCharEq<T>>(C, std::marker::PhantomData<T>);

#[derive(Clone, Debug)]
struct MultiCharEqSearcher<'a, T: Eq, C: MultiCharEq<T>> {
    char_eq: C,
    haystack: &'a [T],
    element_indices: std::iter::Enumerate<std::slice::Iter<'a, T>>,
}

impl<'a, T: 'a + Eq, C: MultiCharEq<T>> Pattern<'a, T> for MultiCharEqPattern<T, C> {
    type Searcher = MultiCharEqSearcher<'a, T, C>;

    #[inline]
    fn into_searcher(self, haystack: &'a [T]) -> MultiCharEqSearcher<'a, T, C> {
        MultiCharEqSearcher {
            haystack,
            char_eq: self.0,
            element_indices: haystack.iter().enumerate(),
        }
    }
}

impl<'a, T: Eq, C: MultiCharEq<T>> Searcher<'a, T> for MultiCharEqSearcher<'a, T, C> {
    #[inline]
    fn haystack(&self) -> &'a [T] {
        self.haystack
    }
    ...
    #[inline]
    fn next(&mut self) -> SearchStep {
        let s = &mut self.element_indices;
...
impl<'a, T: Eq, C: MultiCharEq<T>> ReverseSearcher<'a, T> for MultiCharEqSearcher<'a, T, C> {
    #[inline]
    fn next_back(&mut self) -> SearchStep {
        let s = &mut self.element_indices;
        // Compare lengths of the internal byte slice iterator
...
impl<'a, T: Eq, C: MultiCharEq<T>> DoubleEndedSearcher<'a, T> for MultiCharEqSearcher<'a, T, C> {}
...
macro_rules! pattern_methods {
    ($t:ty, $pmap:expr, $smap:expr) => {
        ...
        #[inline]
        fn into_searcher(self, haystack: &'a [T]) -> $t {
            ($smap)(($pmap)(self, std::marker::PhantomData).into_searcher(haystack))
        }

        #[inline]
        fn is_contained_in(self, haystack: &'a [T]) -> bool {
            ($pmap)(self, std::marker::PhantomData).is_contained_in(haystack)
        }

        #[inline]
        fn is_prefix_of(self, haystack: &'a [T]) -> bool {
            ($pmap)(self, std::marker::PhantomData).is_prefix_of(haystack)
        }

        #[inline]
        fn is_suffix_of(self, haystack: &'a [T]) -> bool
            where $t: ReverseSearcher<'a, T>
        {
            ($pmap)(self, std::marker::PhantomData).is_suffix_of(haystack)
        }
    }
}
...
// /// Delegates to the `&[T]` impl.
// impl<'a, 'b, 'c> Pattern<'a> for &'c &'b [T] {
...
/// boundary.
impl<'a, 'b, T: 'a + Eq + Ord> Pattern<'a, T> for &'b [T] {
    type Searcher = StrSearcher<'a, 'b, T>;

    #[inline]
    fn into_searcher(self, haystack: &'a [T]) -> StrSearcher<'a, 'b, T> {
...
pub struct StrSearcher<'a, 'b, T: Eq> {
    haystack: &'a [T],
    needle: &'b [T],

    searcher: StrSearcherImpl<T>,
}

#[derive(Clone, Debug)]
enum StrSearcherImpl<T: Eq> {
    Empty(EmptyNeedle),
...
impl<'a, 'b, T: Eq + Ord> StrSearcher<'a, 'b, T> {
    fn new(haystack: &'a [T], needle: &'b [T]) -> StrSearcher<'a, 'b, T> {
        if needle.is_empty() {
            StrSearcher {
                haystack,
                needle,
            ...
            StrSearcher {
                haystack,
                needle,
                searcher: StrSearcherImpl::TwoWay(TwoWaySearcher::new(needle, haystack.len())),
            }
        }
    }
}

impl<'a, 'b, T: Eq + Ord> Searcher<'a, T> for StrSearcher<'a, 'b, T> {
    #[inline]
    fn haystack(&self) -> &'a [T] {
        self.haystack
    }
    ...
    fn next(&mut self) -> SearchStep {
        match self.searcher {
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                ...
                let is_long = searcher.memory == std::usize::MAX;
                match searcher.next::<RejectAndMatch>(self.haystack, self.needle, is_long) {
...
impl<'a, 'b, T: Eq + Ord> ReverseSearcher<'a, T> for StrSearcher<'a, 'b, T> {
    #[inline]
    fn next_back(&mut self) -> SearchStep {
        match self.searcher {
            StrSearcherImpl::Empty(ref mut searcher) => {
            ...
            StrSearcherImpl::TwoWay(ref mut searcher) => {
                ...
                let is_long = searcher.memory == std::usize::MAX;
                match searcher.next_back::<RejectAndMatch>(self.haystack, self.needle, is_long) {
...
struct TwoWaySearcher<T: Eq> {
    ...
    _phandom: std::marker::PhantomData<T>,
}
...
*/
impl<T: Eq + Ord> TwoWaySearcher<T> {
    fn new(needle: &[T], end: usize) -> Self {
        ...
        if &needle[..crit_pos] == &needle[period..period + crit_pos] {
            ...
            TwoWaySearcher {
                ...
                memory: std::usize::MAX, // Dummy value to signify that the period is long
                memory_back: std::usize::MAX,
    ...
    #[inline]
    fn next<S>(&mut self, haystack: &[T], needle: &[T], long_period: bool) -> S::Output
    where
        S: TwoWayStrategy,
    {
    ...
    #[inline]
    fn next_back<S>(&mut self, haystack: &[T], needle: &[T], long_period: bool) -> S::Output
    where
        S: TwoWayStrategy,
    {
    ...
    //
    // For long period cases, the resulting period is not exact (it is too short).
    fn reverse_maximal_suffix(arr: &[T], known_period: usize, order_greater: bool) -> usize {
        let mut left = 0; // Corresponds to i in the paper
        let mut right = 1; // Corresponds to j in the paper
        let mut offset = 0; // Corresponds to k in the paper, but starting at 0
                            // to match 0-based indexing.
